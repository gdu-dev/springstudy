...
public class UploadServiceImpl implements UploadService {
  ...
  public int removeUpload(int uploadNo) {
    ...
    return uploadMapper.removeUpload(uploadNo);
  ...
  public int removeUpload_Stream_Lambda(int uploadNo) {
    ...
    if(attachList != null && attachList.isEmpty() == false) {
      ...
      // 삭제할 첨부 파일들
      List<File> fileList = attachList.stream()
                .map(attachDTO -> new File(attachDTO.getPath(), attachDTO.getFilesystemName()))  // attachDTO를 new File()로 변환
                .collect(Collectors.toList());                                                   // 변환된 new File()들을 모두 List로 만들어서 반환
      
      // 삭제할 썸네일들
      attachList.stream()
                .filter(attachDTO -> attachDTO.getHasThumbnail() == 1)                                  // 썸네일을 가진 attachDTO가 작업대상
                .map(attachDTO -> new File(attachDTO.getPath(), "s_" + attachDTO.getFilesystemName()))  // attachDTO를 new File()로 변환
                .forEach(file -> fileList.add(file));                                                   // 변환된 file을 fileList에 추가
  ...
  public int removeAttach(int attachNo) {
    ...
    return uploadMapper.removeAttach(attachNo);
